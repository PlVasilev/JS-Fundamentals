class PublicTransportTable {
    ...
    addEventListeners() {
        $('.search-btn').on('click', function () {
            ...
            if (name || type) {
                let rows = $('.vehicles-info > tr').not('.more-info')// take all tr from .vehicle info class
                console.log(rows)
                for (let i = 0; i < rows.length; i++) {
                    let firstChild = $(rows[i]).find('td').eq(0) // find td in row eq(0) first td
                    let secondChild = $(rows[i]).find('td').eq(1)//find td in row eq(1) second td
                    if(!firstChild.text().includes(type) || !secondChild.text().includes(name)) {
                        $(rows[i]).css('display', 'none')
                        let button = $(rows[i]).find('td').eq(2).find('button') // find 'button' in third td
